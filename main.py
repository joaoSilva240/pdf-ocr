...
def detect_and_format_tables(
    ...
    while i < len(line_profiles):
        if line_profiles[i]["num_cols"] >= 3:
            ...
            header = data_rows_raw[0][:3] if len(data_rows_raw[0]) >= 3 else data_rows_raw[0]
            data_rows = [row[:3] if len(row) >= 3 else row for row in data_rows_raw[1:]]
...
def mask_table_regions(image_path: Path, tables: list[dict]) -> Path:
    ...
    for table in tables:
        ...
        draw.rectangle([(0, max(0, y_start)), (img.width, min(img.height, y_end))], fill="white")
